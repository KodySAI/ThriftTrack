...
interface UpdateTransactionVariables {
  input: Transaction & { transactionId: string };
}

const TransactionPage: React.FC = () => {
  const { id } = useParams<{ id: string }>(); 
  const { loading, data } = useQuery<TransactionQueryResponse, TransactionQueryVariables>(GET_TRANSACTION, {
    variables: { id: id! },
  });

  const [updateTransaction, { loading: loadingUpdate }] = useMutation<void, UpdateTransactionVariables>(
    UPDATE_TRANSACTION,
    {
